...
const list = (el) => {
    ...
    const bookmarks = _converse.bookmarks;

    return html`
        <div class="controlbox-padded list-container list-container--bookmarks ${ bookmarks.length ? 'fade-in' : 'hidden' }">
            <a class="list-toggle bookmarks-toggle"
            ...
            ${bookmarks.map(bm => bookmark_item(bm)) }
            </div>
        </div>
    `;
}
